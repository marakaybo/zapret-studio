...
const CASES: { task: string; answer: string; engine: Engine | null; why: string }[] = [
  ...
  },
  {
    ...
    answer: "zapret",
    engine: "zapret",
    why: "Голос идёт по UDP, а его правит только драйвер. Ни один локальный прокси — ни ByeDPI, ни Xray, ни sing-box — UDP приложений не видит. На экране проверки за это отвечает группа «Голос и UDP»: пока она не зелёная, голоса не будет",
  },
  {
    task: "Игры лагают или не подключаются к серверам",
    answer: "zapret + игровой фильтр",
    engine: "zapret",
    why: "На главной включи игровой фильтр: обход начнёт работать на портах 1024–65535. Сеть нагружается сильнее, зато игровой трафик проходит. Помогло или нет, покажет проверка — группы «Игры» и «Голос и UDP»",
  },
  {
  ...
  {
    task: "Нужно, чтобы через туннель пошли игры и голос Discord",
    answer: "sing-box: весь трафик",
    engine: "singbox",
    why: "Обычный прокси видит только TCP тех приложений, что читают системные настройки. Пресет «Весь трафик: свой сервер» поднимает сетевой адаптер и забирает всё, включая UDP. Без своего сервера этот пресет не заработает — фрагментация UDP не лечит",
  },
  {
...
const RULES: { title: string; text: string }[] = [
  ...
  {
    title: "TUN — это не про обход, а про охват",
    text: "Пресеты «Весь трафик» у sing-box ничего не обходят сами: обход остаётся тем же — фрагментация или свой сервер. TUN меняет другое — что попадает в ядро. Без него ловится только TCP приложений, читающих системный прокси; с ним — весь трафик машины. Поэтому TUN с фрагментацией пробивает ровно так же, как без него, просто шире; а вот TUN со своим сервером — уже настоящий VPN, и через него работает всё, включая голос и игры.",
  },
  {
          ...
          <div className="row" style={{ alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="sub" style={{ fontSize: 12, lineHeight: 1.6 }}>
                ...
                сервером это уже полноценный VPN, в котором работают и голос, и игры. Драйвер
                вшит в само ядро, ставить отдельно ничего не нужно.
                <br />
                <br />
                ...
                Вставь её на вкладке пресетов и выбери «Через свой сервер» — тогда весь трафик
                пойдёт в туннель. Если серверов много, вставь туда же ссылку на подписку: приложение
                заберёт весь список разом, замерит отклик до каждого и даст выбрать ближайший. Сервер приложение не выдаёт и не подбирает: он твой, и скорость с
